...
import {View, Text, StyleSheet, TouchableOpacity, Image , HelperText} from 'react-native';
import {Button, Searchbar, TextInput , ActivityIndicator, DefaultTheme} from 'react-native-paper';
...
class Extra extends Component {
  ...
  get_deets = async () => {
    ...
    if(!num_regex.exec(this.state.number_of_people)){alert("Please enter the number field correctly!"); return;}
    let a = this.state.start_date
    let b = this.state.end_date
    if(a>b){alert("Please enter correct dates!"); return;}
    await this.props.add_user_deets_to_new({number_of_people : this.state.number_of_people , start_date : this.state.start_date , end_date : this.state.end_date});
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.autocompleteContainer}>
            ...
            />
            <Button labelStyle={{color : "#1e5f74"}} style = {{marginTop:2}}>Enter Start Date</Button>
            <DatePicker
